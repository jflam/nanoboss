...
const AGENT_MODEL_CATALOG: Record<DownstreamAgentProvider, AgentCatalogEntry> = {
  claude: {
    ...
    label: "Claude",
    models: [
      { id: "default", name: "Default", description: "Account-dependent default model" },
      { id: "opus", name: "Opus", description: "Opus 4.5 - Most capable for complex reasoning" },
      { id: "sonnet", name: "Sonnet", description: "Sonnet 4.5 - Best for everyday tasks" },
      { id: "haiku", name: "Haiku", description: "Haiku 4.5 - Fastest for quick answers" },
      { id: "opusplan", name: "Opus Plan", description: "Hybrid: Opus for planning, Sonnet for execution" },
    ],
  },
  gemini: {
    provider: "gemini",
...
export function listSelectableModelOptions(provider: DownstreamAgentProvider): SelectableModelOption[] {
...
export function findSelectableModelOption(
  ...
  return listSelectableModelOptions(provider).find((option) => option.value === trimmed);
}
