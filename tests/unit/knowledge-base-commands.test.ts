...
describe("knowledge-base procedures", () => {
  ...
  test("/kb-refresh composes ingest and source compilation", async () => {
    const cwd = createWorkspace();
    ...
    const data = result.data as { compiledSourceIds: string[]; sourceCount: number };
    expect(data.sourceCount).toBe(1);
    ...
    expect(readFileSync(join(cwd, "wiki", "log.md"), "utf8")).toContain("refresh | 1 source(s) updated");
